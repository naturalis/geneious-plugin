...
public class GlobalOptions extends Options {
  ...
  private void addGeneralOptions() {
    ...
    IntegerOption querySize = addIntegerOption(QUERY_SIZE.getName(), "Max. query size", 100, 10, 1000);
    querySize.setIncrement(10);
    querySize.setHelp("If you are getting error messages like \"Error while querying database\", it may be that the "
        + "database is choking on the query generated by the plugin (especially when importing large sample sheets). "
        + "In that case try lowering this setting.");
    querySize.addChangeListener(() -> settings().update(QUERY_SIZE, querySize.getValue()));
    settings().update(QUERY_SIZE, querySize.getValue());
    ...
    clearPingdata.setHelp("Press this button if you accidentally deleted a \"ping\" folder while waiting for document "
        + "indexing to complete. Make sure to manually check all documents have been indexed (or just wait a reasonable "
        + "amount of time) before continuing to use the plugin");
  ...
  private void addLoggingOptions() {
    ...
    BooleanOption debug = addBooleanOption(DEBUG.getName(), "Enable DEBUG mode", FALSE);
    ...
    BooleanOption prettyNotes = addBooleanOption(PRETTY_NOTES.getName(), "Pretty notes", FALSE);
  ...
  private void addBoldOptions() {
    ...
    MultipleLineStringOption markerMap = addMultipleLineStringOption(MARKER_MAP.getName(), "Marker mappings", defaultMap, 6, false);
