...
/**
 * Pings the database to ascertain that all documents have been indexed. The mechanism is that, after an operation is
 * done creating or updating documents, the {@code Ping} object sends one more fasta-like document to the database with
 * a special {@link NaturalisField#SMPL_EXTRACT_ID extract ID} and then waits until it gets that document back by
 * querying on that extract ID. That is taken to indicate that all preceding documents have been indexed as well. This
