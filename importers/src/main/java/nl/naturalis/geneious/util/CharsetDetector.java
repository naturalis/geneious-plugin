...
/**
 * <p>
 * Utility class to guess the encoding of a given byte buffer. The guess is unfortunately not 100% sure, especially for
 * 8-bit charsets. It's not possible to know which 8-bit charset is used. Except through statistical analysis. We only
 * try to detect from 3 widely used 8bit encodings: latin1, windows 1252 and macroman.
 * </p>
 *
 * <p>
...
public class CharsetDetector {
   ...
   */
  private CharsetDetector(ByteBuffer buffer) {
    this.buffer = buffer;
  }
  ...
  public static Charset detectEncoding(ByteBuffer buffer) {
    Charset charset = new CharsetDetector(buffer).detectEncoding();
    return charset;
  }
   ...
   * <p>
   * 2 bytes: 110xxxxx 10xxxxxx 3 bytes: 1110xxxx 10xxxxxx 10xxxxxx 4 bytes: 11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
   * <p>
   * The amount of ones in the first byte tells you how many of the following bytes still belong to the same character.
   * All bytes that belong to the sequence start with 10 in binary. To encode the character you convert its codepoint to
   * binary and fill in the x's.
   *
   ...
   * this BOM. Otherwise, the file would not be a human readable text file.
   * </p>
   * <p/>
   * <p>
   * If there is no BOM, this method tries to discern whether the file is UTF-8 or not. If it is not UTF-8, we try to
   * select the best match from latin1, windows 1252 or macroman.
   * </p>
   * <p/>
   * <p>
  ...
  /**
   * Spot suspicous characters which are rarely used in our texts see http://de.wikipedia.org/wiki/ISO_8859-1
   */
  private long testWindows1252() {
    return testDecoder(WINDOWS1252.newDecoder(),
        b -> (b == (byte) 0x81 || b == (byte) 0x8d || b == (byte) 0x8f || b == (byte) 0x90 || b == (byte) 0x9d));
  }
