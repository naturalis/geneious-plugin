...
class SampleSheetSelector2 {

  private final SampleSheetProcessor processor;
  private final AnnotatedPluginDocument[] selectedDocuments;

  private JDialog dialog;
  private JTextField sampleSheetTextField;
  private JTextField sheetNoTextField;
  private JTextField skipLinesTextField;

  SampleSheetSelector2(SampleSheetProcessor processor, AnnotatedPluginDocument[] docs) {
  ...
  void show() {
    ...
    JLabel label = new JLabel("Sample sheet", SwingConstants.RIGHT);
    panel0.add(label, c);
    ...
    c.gridy = 0;
    c.weightx = 1f;
    sampleSheetTextField = new JTextField(50);
    panel1.add(sampleSheetTextField, c);
    ...
    browseButton.addActionListener(new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        if (fc.showOpenDialog(dialog) == JFileChooser.APPROVE_OPTION) {
          ...
          if (f != null) {
            sampleSheetTextField.setText(f.getAbsolutePath());
  ...
  private JButton createOkButton() {
    ...
    okButton.addActionListener(new ActionListener() {
      @Override
      public void actionPerformed(ActionEvent e) {
        if (StringUtils.isBlank(sampleSheetTextField.getText())) {
          JOptionPane.showMessageDialog(dialog, "Please select a sample sheet",
              "No sample sheet selected", JOptionPane.ERROR_MESSAGE);
        }
        else {
          File sampleSheet = new File(sampleSheetTextField.getText());
          if (!sampleSheet.isFile()) {
            JOptionPane.showMessageDialog(dialog, "Invalid file: " + sampleSheetTextField.getText(),
                "Invalid file", JOptionPane.ERROR_MESSAGE);

          }
          else if (selectedDocuments.length==0 /* && dummies */) {
            ...
            int i;
            try {
              i = Integer.parseInt(skipLinesTextField.getText());
            } catch (NumberFormatException exc) {

            }
            dialog.dispose();
            SampleSheetProcessingOptions options = new SampleSheetProcessingOptions();
